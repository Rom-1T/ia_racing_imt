...
DATASET_DIR = 'supervise/dataset_drive/'

LEARNING_RATE=0.002
WIDTH = 160
HEIGHT = 120
BATCH_SIZE = 3
NUMBER_OF_IMAGES = 81850
...
def get_images_for_new_batch(batch_number, image_in_batch):
    ...
    img_arr = cv2.imread(DATASET_DIR+dataset_images[imgN])
...
Net.fc = torch.nn.Linear(in_features=512, out_features=2, bias=True) # Change final layer to predict one value
Net = Net.to(device)
...
with alive_bar(bar_total) as bar:
    for batch_N in range(NUMBER_OF_BATCHES): # Training loop
        ...
        print(batch_N,") Loss=",Loss.data.cpu().numpy()) # Display loss
