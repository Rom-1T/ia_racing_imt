...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    class SpeedMulitplicator:
        def __init__(self, cfg):
            self.factor = cfg.SPEED_MULTIPLIER 
        def run(self, thr):
            return thr * self.factor
    
    #V.add(SpeedMulitplicator(), inputs=['user/mode'], outputs=['run_pilot'])
    ...
    if model_path:
        ...
        V.add(SpeedMulitplicator(cfg), inputs=['pilot/throttle'], outputs=['pilot/throttle'], run_condition='run_pilot')
    ...
    tub_writer = TubWriter(tub_path, inputs=inputs, types=types, metadata=meta, max_catalog_len=cfg.MAX_CATALOG_LEN)
...
def add_user_controller(V, cfg, use_joystick, input_image='cam/image_array', input_prepro_image = 'prepro/image_lines'):
    ...
    if cfg.DONKEY_GYM:
        inputs=[input_image, input_prepro_image, 'tub/num_records', 'user/mode', 'recording', 'gym/throttle', 'gym/angle', 'lap', 'end', 'start']
    else:
        # class PrintThrottleAngle():
        #     def run(self, thr, ang):
        #         print("Throttle :", thr, "  - Angle :", ang)
        # V.add(PrintThrottleAngle(), ['throttle', 'angle'])
        
        inputs=[input_image, input_prepro_image, 'tub/num_records', 'user/mode', 'recording', 'throttle', 'angle', 'lap', 'end', 'start']
    V.add(ctr,
          inputs,
          outputs=['user/angle', 'user/throttle', 'user/mode', 'recording', 'web/buttons', 'stop_the_stop', 'start'],
          threaded=True)
...
def add_simulator(V, cfg):
    ...
    if cfg.DONKEY_GYM:
        ...
        gym = DonkeyGymEnv(cfg.DONKEY_SIM_PATH, host=cfg.SIM_HOST, env_name=cfg.DONKEY_GYM_ENV_NAME, conf=cfg.GYM_CONF,
                           ...
                           record_velocity=cfg.SIM_RECORD_VELOCITY, record_lidar=cfg.SIM_RECORD_LIDAR, record_speed = cfg.SIM_SPEED_VEHICULE,
        ...
        if cfg.SIM_SPEED_VEHICULE:
            outputs += ['gym/throttle']
