var driveHandler = new function() {
            ...
            keys.forEach(key => {
                ...
                if(state.hasOwnProperty(key) && state[key] !== data[key]) {
                    if(typeof state[key] === 'object') {
                        ...
                        console.log("Changement pour", key, "de", state[key], "à", data[key] )
                        state[key] = data[key];
                        changed = true;
                      }
                    }
    ...
    var updateUI = function() {
      ...
      if (state.throttle) {
        $('#v-throttle-value').html(parseFloat(state.throttle).toFixed(2));
      }

      if (state.angle) {
        $('#v-angle-value').html(parseFloat(state.angle).toFixed(2));
      }
      ...
      if(state.end) {
        ...
        let confetti = new Confetti('end-confetti');
