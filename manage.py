...
from donkeycar.parts.pytorch import PytorchReinforcment
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if cfg.PREPROCESSING:
        ...
        V.add(prepro, inputs=['cam/image_array'], outputs=['prepro/image_array'])
    ...
    if cfg.USE_FPV:
        V.add(WebFpv(), inputs=['prepro/image_array'], threaded=True)
    ...
    if model_type == "pytorch":
        if model_path:
            model = PytorchReinforcment(cfg)
            model.load(model_path)
            if cfg.RECONSTRUCT :
                V.add(model, inputs=['prepro/image_array'], outputs=['pilot/angle','pilot/throttle','reconstructed_image'], run_condition='run_pilot',threaded=True)
            else :
                V.add(model, inputs=['prepro/image_array'],
...
def add_user_controller(V, cfg, use_joystick, input_image='cam/image_array'):
    ...
    if cfg.RECONSTRUCT :
        V.add(ctr,
              inputs=['reconstructed_image', 'tub/num_records', 'user/mode', 'recording'],
              outputs=['user/angle', 'user/throttle', 'user/mode', 'recording', 'web/buttons'],
              threaded=True)
    else :
        V.add(ctr,
              inputs=['prepro/image_array', 'tub/num_records', 'user/mode', 'recording'],
              outputs=['user/angle', 'user/throttle', 'user/mode', 'recording', 'web/buttons'],
              threaded=True)
